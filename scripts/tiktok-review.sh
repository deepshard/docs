...
try:
    ...
    assert str(state.get('client_user_id')) == '35598109'
...
reviewer_main() {
    ...
    case "${1:-}" in
        --help|-h)
            echo 'Usage: bash tiktok-review.sh [--reconnect]'
    ...
    reviewer_root="$HOME/.local/share/truffle-tiktok-review"
...
PY
    ...
    if [[ "$reviewer_reconnect" == true ]] || ! reviewer_has_connection; then
        echo 'Paste the private Truffle invitation supplied for this TikTok review.'
    ...
    if "$reviewer_python" -c \
        'import json,sys; sys.exit(0 if any(a["name"].casefold() == "tiktok" for a in json.load(sys.stdin)["apps"]) else 1)' <<< "$reviewer_catalog"; then
        echo 'Using the TikTok connection already installed in the review workspace.'
    else
        reviewer_cli install tiktok
    fi
    echo 'Try: Use TikTok to show my profile and account stats.'
    reviewer_cli convo
}
