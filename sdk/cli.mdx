...
Run `/apps` to see installed apps on the connected device:

```
you> /apps

installed apps:
  • Kalshi (org.deepshard.kalshi)
  • Slack (org.deepshard.slack)
  • GitHub (org.deepshard.github)
```

Apps are no longer registered as chat slash commands. The agent discovers
installed app tools through tool search and decides which tools are relevant
for the task.

For scripted one-shot calls where you explicitly want to constrain the task to
an app, use `truffile chat --app <name-or-uuid> "..."`.
...
| **Apps** | Agent can discover installed app tools with tool search | No app integration |
