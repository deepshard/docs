...
## Let Claude build the app for you

If you're using Claude Code (or any Claude-powered agent) in this repo, there's a built-in skill that walks through building a Truffle app end-to-end: **`truffle-app-creator`** (at `skills/SKILL.md`).
...
Steps run during installation on the device, in order. Today the supported types are:

| Type | What it does |
|------|-------------|
| `welcome` | Display markdown content to the user before install begins |
...
Each field in a `text` step maps user input to an environment variable available on your foreground/background processes:

```yaml
fields:
  - name: api_key           # internal field name
    label: API Key          # label shown to the user
    type: password          # text | password
    placeholder: sk_...     # optional hint
    env: MY_APP_API_KEY     # env var the process reads
```

Add a `validator` bash step to verify credentials immediately after the user enters them — the install flow fails with your `error_message` if the bash command exits non-zero.
